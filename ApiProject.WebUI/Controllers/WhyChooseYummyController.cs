...
using ApiProject.WebUI.Dtos.WhyChooseYummyDtos;
...
namespace ApiProject.WebUI.Controllers
{
    public class WhyChooseYummyController : Controller
    {
        ...
        public async Task<IActionResult> WhyChooseYummyList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7123/api/Services");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(createWhyChooseYummyDto);  //metinden jsona =======> deserialize ise jsondan metine
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PostAsync("https://localhost:7123/api/Services", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7123/api/Services?id=" + id);
            return RedirectToAction("WhyChooseYummyList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7123/api/Services/GetService?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateWhyChooseYummy(UpdateWhyChooseYummyDto updateWhyChooseYummyDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7123/api/Services", stringContent);
            return RedirectToAction("WhyChooseYummyList");
        }
    }
}
