...
namespace ApiProject.WebUI.Controllers
{
    public class ChefController : Controller
    {
        ...
        public async Task<IActionResult> ChefList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7123/api/Chefs");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(createChefDto);  //metinden jsona =======> deserialize ise jsondan metine
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PostAsync("https://localhost:7123/api/Chefs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7123/api/Chefs?id=" + id);
            return RedirectToAction("ChefList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7123/api/Chefs/GetChef?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateChef(UpdateChefDto updateChefDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7123/api/Chefs", stringContent);
            return RedirectToAction("ChefList");
        }
    }
}
